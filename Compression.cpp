...
std::string Compression::FormatOutput(int XPos, int RowNum, int LayerNum, int NumX, int NumY, int NumZ, char Ch, std::unordered_map<char, std::string> TagTable) {
...
std::string Compression::SingleLineCompress(const std::string Row, std::unordered_map<char, std::string> TagTable, int ParentX, int ParentY, int ParentZ, int RowNum, int LayerNum) {
  size_t Count = 0;
  ...
  while (Count < Row.size()) {
    ...
    while (Num > 0) {
      ...
      Output += FormatOutput(XPos, RowNum, LayerNum, Chunk, 1, 1, Ch, TagTable);
      // advance x position
      XPos += Chunk;
      Num -= Chunk;
    }
...
std::vector<Block> Compression::MergeRows(const std::vector<Block> &PrevRow, const std::vector<Block> &CurrRow, int ParentY) {
...
std::string Compression::WriteBlocks(const std::vector<Block> &Blocks, const std::unordered_map<char, std::string> &TagTable) {
  std::string Output;
  for (const auto &B : Blocks) {
    Output += FormatOutput(B.XPos, B.YPos, B.ZPos, B.XSize, B.YSize, B.ZSize, B.Ch, TagTable);
  }
  return Output;
}

std::string Compression::ProcessLayer(const std::vector<std::string> &Rows, int ParentX, int ParentY, int ParentZ, int LayerNum, const std::unordered_map<char, std::string> &TagTable) {
  ...
  for (int RowNum = 0; RowNum < Height; RowNum++) {
    ...
    auto CurrRow = SingleLineBlocks(Rows[YPos], ParentX, ParentY, ParentZ, YPos, LayerNum);
    ...
    if ((RowNum + 1) % ParentY == 0 || RowNum == Height - 1) {
      Output += WriteBlocks(Accumulated, TagTable);
      Accumulated.clear();
    }
  }
  return Output;
