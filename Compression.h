...
public:
  ...
  std::vector<Block> MergeRows(std::vector<Block>& prevRow, std::vector<Block>& currRow, int ParentY);
  ...
  bool TryRelaxedMerge(Block& prev, Block& curr, int ParentY, std::vector<Block>& currLeftovers, std::vector<Block>& Leftovers);
};
